...
"""
function threadinfo_pluto(; kwargs...)
    ti = ThreadInfo(kwargs)
    return ti
end

Base.show(io::IO, ti::ThreadInfo) = threadinfo(io; ti.kwargs...)

function threadinfo(io::IO = stdout; blas = false, hints = false, color = true,
                    masks = false,
...
function _visualize_affinity(io = stdout;
                             thread_cpuids = getcpuids(),
                             blocksize = 16,
                             color = true,
                             groupby = :sockets,
